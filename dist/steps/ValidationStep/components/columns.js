...
import { Autocomplete } from '../../../components/Fields/Autocomlete/Autocomplete.js';
...
const generateColumns = (fields) => [
    ...
    ...fields.map((column) => ({
        ...
        minWidth: 150,
        resizable: true,
        headerRenderer: () => (jsxs(Box, { display: "flex", gap: 1, alignItems: "center", position: "relative", children: [jsx(Box, { flex: 1, overflow: "hidden", textOverflow: "ellipsis", children: column.label }), column.description && (jsx(Tooltip, { placement: "top", hasArrow: true, label: column.description, children: jsx(Box, { flex: "0 0 auto", children: jsx(CgInfo, { size: "16px" }) }) }))] })),
        ...
        editor: ({ row, onRowChange, onClose }) => {
            ...
            switch (column.fieldType.type) {
                case "select":
                    ...
                    break;
                case "autocomplete":
                    component = (jsx(Box, { paddingInlineStart: "0.5rem", children: jsx(Autocomplete, { searcher: column.fieldType.searcher, placeholder: column.fieldType.placeholder, formatOption: column.fieldType.formatOption, value: row[column.key], onSelect: (id) => {
                                onRowChange({ ...row, [column.key]: id }, true);
                                onClose(true);
                            } }) }));
                    break;
                default:
                    component = (jsx(Box, { paddingInlineStart: "0.5rem", children: jsx(Input, { ref: autoFocusAndSelect, variant: "unstyled", autoFocus: true, size: "small", value: row[column.key], onChange: (event) => {
            ...
            switch (column.fieldType.type) {
                ...
                case "autocomplete":
                    const selected = column.fieldType.formatOption
                        ? column.fieldType.formatOption(column.fieldType.initialOptions?.find(i => i.id === row[column.key]))
                        : column.fieldType.initialOptions?.find(i => i.id === row[column.key])?.name;
                    component = (jsx(Box, { minWidth: "100%", minHeight: "100%", overflow: "hidden", textOverflow: "ellipsis", children: selected || "" }));
                    break;
                default:
