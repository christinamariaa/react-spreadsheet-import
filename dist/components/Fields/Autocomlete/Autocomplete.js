...
import { Box, Input, IconButton, List, ListItem, Spinner } from '@chakra-ui/react';
...
const Autocomplete = ({ searcher, onSelect, placeholder, pageSize = 20, hasPagination = false, formatOption, value, initialOptions = [], }) => {
    ...
    const [allItems, setAllItems] = useState(initialOptions);
    ...
    const getDisplayText = useCallback((item) => (formatOption ? formatOption(item) : item.name), [formatOption]);
    useEffect(() => {
    ...
    const getPaginatedItems = useCallback((items, query, currentOffset) => {
        let filtered = items;
        if (query) {
    ...
    return (jsxs(Box, { position: "relative", w: "100%", children: [jsxs(Box, { position: "relative", children: [jsx(Input, { ref: inputRef, placeholder: placeholder, value: inputValue, onChange: (e) => setInputValue(e.target.value), onFocus: () => {
                            if (inputValue === "" && suggestions.length === 0) {
                                loadInitialItems();
                            }
                            else
                                setIsOpen(true);
                        }, onBlur: () => setTimeout(() => setIsOpen(false), 200), paddingRight: "32px" }), inputValue && (jsx(IconButton, { "aria-label": "clear", 
                        // icon={<CloseIcon boxSize={3} />}
                        onClick: () => {
                            setInputValue("");
                            selectedItemRef.current = null;
                            onSelect(null);
                        }, size: "xs", variant: "ghost", position: "absolute", top: "50%", right: "6px", transform: "translateY(-50%)" }))] }), isOpen && (jsxs(List, { ref: listRef, position: "absolute", top: "100%", left: "0", right: "0", bg: "white", borderWidth: "1px", borderColor: "gray.200", borderTop: "none", borderRadius: "0 0 6px 6px", maxH: "200px", overflowY: "auto", zIndex: 10, boxShadow: "md", onScroll: handleScroll, children: [suggestions.map((item) => (jsx(ListItem, { px: 3, py: 2, borderBottomWidth: "1px", borderColor: "gray.100", cursor: "pointer", _hover: { bg: "gray.50" }, onMouseDown: () => handleSelect(item), children: getDisplayText(item) }, item.id))), isLoadingMore && (jsx(ListItem, { textAlign: "center", py: 2, children: jsx(Spinner, { size: "sm" }) })), !hasMore && suggestions.length > 0 && (jsx(ListItem, { textAlign: "center", py: 2, color: "gray.500", children: "\u041F\u043E\u043A\u0430\u0437\u0430\u043D\u044B \u0432\u0441\u0435 \u044D\u043B\u0435\u043C\u0435\u043D\u0442\u044B" })), !isLoading && suggestions.length === 0 && (jsx(ListItem, { textAlign: "center", py: 2, color: "gray.400", children: "\u041D\u0438\u0447\u0435\u0433\u043E \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u043E" }))] }))] }));
};
